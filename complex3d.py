...
def contourfunc(x):
    return lin_contourfunc(
        x)  # Pick one of the functions defined below, or a product such as lin_contourfunc(x)*log_contourfunc(x) to overlay multiple
...
phi, theta = np.mgrid[0:pi:(resolution + 1) * 1j,
             -pi:pi:(resolution + 1) * 1j]  # complex step number just makes it inclusive
...
if polarPlane:
    ...
    reZ, imZ = np.mgrid[-trimMag:trimMag:(resolution + 1) * 1j,
               -trimMag:trimMag:(resolution + 1) * 1j]  # complex step number just makes it inclusive
    trimmedZ = reZ + 1j * imZ
    trimmedZeros = np.zeros_like(np.real(trimmedZ))
...
class MyaviModel(HasTraits):
    ...
    # update the plot.
    @on_trait_change(
        'idInclSphereArg,idInclSphereMag,idInclPlaneArg,idInclPlaneMag,idScene.activated,fInclSphereArg,fInclSphereMag,fInclPlaneArg,fInclPlaneMag,fScene.activated')
    def update_plot(self):
        # initialize the plots if unintialized
        if (self.idSphereArgPlot is None) or (self.idSphereMagPlot is None) or (self.idPlaneArgPlot is None) or (
                self.idPlaneMagPlot is None) or (self.fSphereArgPlot is None) or (self.fSphereMagPlot is None) or (
                self.fPlaneArgPlot is None) or (self.fPlaneMagPlot is None):
            # id meshes
            self.idSphereArgPlot = self.idScene.mlab.mesh(cartX, cartY, cartZ, scalars=theta, vmin=-pi, vmax=pi,
                                                          figure=self.idScene.mayavi_scene)
            self.idSphereMagPlot = self.idScene.mlab.mesh(cartX, cartY, cartZ, scalars=contourfunc(z),
                                                          figure=self.idScene.mayavi_scene)
            self.idPlaneArgPlot = self.idScene.mlab.mesh(np.real(trimmedZ), np.imag(trimmedZ), trimmedZeros,
                                                         scalars=trimmedTheta, figure=self.idScene.mayavi_scene)
            self.idPlaneMagPlot = self.idScene.mlab.mesh(np.real(trimmedZ), np.imag(trimmedZ), trimmedZeros,
                                                         scalars=contourfunc(trimmedZ),
                                                         figure=self.idScene.mayavi_scene)
            # f meshes
            self.fSphereArgPlot = self.fScene.mlab.mesh(cartX, cartY, cartZ, scalars=np.angle(f(z)), vmin=-pi, vmax=pi,
                                                        figure=self.fScene.mayavi_scene)
            self.fSphereMagPlot = self.fScene.mlab.mesh(cartX, cartY, cartZ, scalars=contourfunc(f(z)),
                                                        figure=self.fScene.mayavi_scene)
            self.fPlaneArgPlot = self.fScene.mlab.mesh(np.real(trimmedZ), np.imag(trimmedZ), trimmedZeros,
                                                       scalars=np.angle(f(trimmedZ)), figure=self.fScene.mayavi_scene)
            self.fPlaneMagPlot = self.fScene.mlab.mesh(np.real(trimmedZ), np.imag(trimmedZ), trimmedZeros,
                                                       scalars=contourfunc(f(trimmedZ)),
                                                       figure=self.fScene.mayavi_scene)
        ...
        if (not self.idInclSphereArg):
            ...
            self.idSphereArgPlot.mlab_source.set(z=np.zeros_like(cartX), x=np.zeros_like(cartX), y=np.zeros_like(
                cartX))  # didn't work: transparent=True, opacity=0.5, figure=self.idScene.mayavi_scene, zmax=0.5
        ...
        if (not self.idInclSphereMag):
            ...
            self.idSphereMagPlot.mlab_source.set(z=np.zeros_like(cartX), x=np.zeros_like(cartX), y=np.zeros_like(
                cartX))  # didn't work: transparent=True, opacity=0.5, figure=self.idScene.mayavi_scene, zmax=0.5
        else:  # spheres need to shrink out of the way (half-baked bug fix)
        ...
        if (not self.fInclSphereArg):
            ...
            self.fSphereArgPlot.mlab_source.set(z=np.zeros_like(cartX), x=np.zeros_like(cartX), y=np.zeros_like(
                cartX))  # didn't work: transparent=True, opacity=0.5, figure=self.idScene.mayavi_scene, zmax=0.5
        else:  # spheres need to shrink out of the way (half-baked bug fix)
...
idView = View(Item('idScene', editor=SceneEditor(scene_class=MayaviScene), height=250, width=300, show_label=False),
              # , show_label=False),
              Group(
...
fView = View(Item('fScene', editor=SceneEditor(scene_class=MayaviScene), height=250, width=300, show_label=False),
             # , show_label=False),
             Group(
